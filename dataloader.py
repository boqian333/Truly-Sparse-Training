...
class Data(object):
    ...
    def __init__(self, x_train, y_train, x_test, y_test, batch_size, augmentation=False, dataset=''):
        self.x_train = x_train
        self.y_train = y_train
        self.x_test = x_test
        self.y_test = y_test
        self.batch_size = batch_size
        # self.augmentation = (augmentation and (dataset == 'cifar10' or dataset == 'cifar100'))
    ...
    def get_train_data(self):
        if self.augmentation:
            if 'cifar' in self.dataset:
                x_b = self.x_train.reshape(-1, 32 * 32 * 3)
            else:
                x_b = self.x_train.reshape(-1, 28 * 28)
            return x_b
        else:
            return self.x_train
    ...
    def get_test_data(self):
        if self.augmentation:
            if 'cifar' in self.dataset:
                x_b = self.x_test.reshape(-1, 32 * 32 * 3)
            else:
                x_b = self.x_test.reshape(-1, 28 * 28)
            return x_b
        else:
            return self.x_test
