...
# FixProb-MLP
for i in range(5):
    with open('../Pretrained_results/rigl_mlp_5000_training_samples_e13_rand' + str(i) + '_monitor.json') as json_file:
        data = np.array(json.load(json_file))
...
ax1.plot(np.arange(0, len(memory_usage_fixprob_mlp)/2, 0.5), memory_usage_fixprob_mlp.rolling(window=5).mean(), label="RigL-MLP memory usage", color="g")
...
ax2.plot(np.arange(0, len(cpu_usage_fixprob_mlp)/2, 0.5), cpu_usage_fixprob_mlp.rolling(window=5).mean(), label="RigL-MLP cpu usage", color="g")
...
samples = 5000
plt.savefig("../Results/fashionmnist_memory_and_cpu_usage_rigl_samples"+str(samples)+".pdf", bbox_inches='tight')
